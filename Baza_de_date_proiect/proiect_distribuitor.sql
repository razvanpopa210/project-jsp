...
CREATE TABLE `distribuitor` (
  `DistribuitorID` int(11) NOT NULL AUTO_INCREMENT,
  `ProdusID` int(11) NOT NULL,
  `ProducatorID` int(11) NOT NULL,
  `Nume` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`DistribuitorID`),
  KEY `fk_1_idx` (`ProdusID`),
  KEY `foreign_key_1_idx` (`ProducatorID`),
  CONSTRAINT `foreign_key_1` FOREIGN KEY (`ProducatorID`) REFERENCES `producator` (`ProducatorID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `foreign_key_2` FOREIGN KEY (`ProdusID`) REFERENCES `produsalimentar` (`ProdusID`) ON DELETE CASCADE ON UPDATE CASCADE
